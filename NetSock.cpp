...
//
// LICENSE
//   Copyright 2007 Gynvael Coldwind
//
...
int 
NetSock::WriteAll(unsigned char *Buffer, int Size)
{
  if(this->socket == -1)
    return -1;
  ...
  while(ptr != Size)
  {
    ret = send(this->socket, (char*)Buffer + ptr, Size - ptr, 0);
    if(ret == 0) 
      return 0;
    else if(ret == -1) 
...
int
NetSock::BroadcastUDP(const char* broadcast, unsigned short port, void *buffer, int size)
{
  // Sanity check
  if(this->socket == -1)
    return -1;
